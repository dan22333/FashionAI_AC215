...
def test_get_image_data(mock_storage_client):
    ...
    mock_blob.download_as_bytes.return_value = BytesIO(b"fake_image_data").getvalue()
    mock_bucket = MagicMock()
...
def test_process_image_metadata(mock_pinecone):
    ...
    metadata_df = pd.DataFrame([
        {"source/id": 1, "brand": "Brand A", "medias/0/url": "https://example.com/image.jpg"}
    ])
    mock_index = MagicMock()
    mock_pinecone.return_value.Index.return_value = mock_index
    ...
    with patch("main.get_image_data") as mock_get_image, \
        ...
        mock_get_vector.return_value = Mock(tolist=Mock(return_value=[0.1] * VECTOR_DIM))
...
def test_process_and_upload_topic_parallel(mock_storage_client, mock_pinecone):
    ...
    mock_bucket.list_blobs.return_value = [mock_blob_caption, mock_blob_metadata]
    mock_storage_client.return_value.bucket.return_value = mock_bucket
